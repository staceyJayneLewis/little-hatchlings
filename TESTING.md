...
I have tested all of the following features on all the mobile, tablets, laptops and computers​ that I have at home but also shared my github link with friends and family so they can try it on their mobile and tablet devices, as they all have different branded devices, which are iPhones, android, and Google phones. I also tried the website on my Apple Mac laptop and Windows computer to see how they work on larger devices and on different browsers. The browsers that I tried my live website on are Google chrome, Brave, firefox and Safari. 

The logo in the navbar is responsive across all of the above devices and the link also works to return to the homepage on each of the browsers sites too as it successfully directs the user to the home page (index.html) no matter which device it is tested on. The navbar collapses at the correct breakpoints for each of the device and browsers tested on and the dropdown works very well on the different mobile screens regardless of the brand of phone. Each navbar link works well when clicked and the response time to change to the selected page was lagging a little on my windows laptop but judging by the lighthouse results is mainly due to the js, fontawsome and lighbox however did not lag on any of the other laptop/computers tested on, mobile was quick response. I also tested to see if the navbar works successfully on each page and if the active key style corresponds with the correct page the user clicks on and it does work as expected again, across the same browser and devices.

When checking the hero image and image border down to the 320px responsiveness on the developer tools both elements stay within it's container and is fully responsive with each breakpoint and column change across all of the above stated browsers and devices. The border and image also change at the correct breakpoint sizes for the larger device screens. You can also see using the default phone templates on the developer tools that the images and borders are hidden or shown when expected to.
For the hero header, I have again used the developer tools to check that the heading is responsive and that it does not overflow at certain breakpoint or screen sizes and it stays within the container as expected. The 'Contact Us' button on the callout heading works on all of the above-stated browsers and devices including the button hover styling as tested by friends, family and myself. Also, the text including the header is centered on the small and medium sized screen but then aligned to the left on larger screens as it should behave on all screen sizes and browsers too, you can also see this on the 'am i repsponsive' screenshot or link in the readme.
...
For the video element and the 'About Us' section, I have tested the responsiveness and also checked that the video does not autoplay across all browsers and devices stated. The video behaves as expected on mobile, tablet, laptop and computer and changes into 2 columns with the about section next to it as expected in the same row. The video itself plays through well on all devices and browsers from start to finish and can be paused and rewinded whenever the user wants to, according to the tests I conducted. The video does slightly lose its quality to the end of the video, but that is due to the compression to be able to upload to codeanywhere, it is not due to the browser or devices. The 'Find a nursery' button in the about section and the link attached to the button work successfully by taking the user to the contact us page when clicked, regardless of which device and browser used when tested, again including the shadow feature when hovered over the button. The about section also stays centered within the container column no matter what browser or device screen size too.
...
The 'Our Values' section also stays centered within the container column, no matter what browser or device screen size, and the button link successfully takes the user to the gallery page and the shadow when hovering over the button also works as expected across all browsers & devices. The our values image is also fully responsive regardless of browser and device used and, no matter what size screen the user uses, you can still see the image as it has been styled so it focuses on the middle of the page. 
...
I've tested my deployed project using the Lighthouse Audit tool to check for any major issues. I noticed when doing the testing that the only low performance was on the mobile devices and when looking to see why this is the case and it seems to be js or the fontawsome link mainly. Also, they do suggest on there that I could use different sized images for different media queries however my mentor told me that was not necessary for this project. However, there was also a suggestion to convert my jpg's/png's to webp file to help performance on the mobile and I did this which resulted is slightly better speed however still not great for the mobile currently. 

| Page | Size | Screenshot | Notes |
| --- | --- | --- | --- |
| Home | Mobile | ![screenshot](documentation/mobile-home-lighthouse.jpg) | Few warnings |
| Home | Desktop | ![screenshot](documentation/desktop-home-lighthouse.jpg) | good performance |
| Gallery | Mobile | ![screenshot](documentation/mobile-gallery-lighthouse.jpg) | Few warnings |
| Gallery | Desktop | ![screenshot](documentation/desktop-gallery-lighthouse.jpg) | Some minor warnings |
| Contact | Mobile | ![screenshot](documentation/mobile-contact-lighthouse.jpg) | | Some minor warnings
| Contact | Desktop | ![screenshot](documentation/desktop-contact-lighthouse.jpg) | good performance |
...
- Design not going to the edge of the page even though I have used container fluid

    ![screenshot](documentation/bug01.jpg)

    - To fix this, I added a row as I realised that bootstrap does not work properly if the format container, row, column is not used when using the grid system.


- Video not responsive to the container height when changing size of the screen

    ![screenshot](documentation/bug03.jpg)

    - To fix this, I added a height of 100% and used the object-fit: fill property value to fill the container.


- Strong element showing around the footer element on dev tools but unsure why.

    ![screenshot](documentation/bug04.jpg)

    - To fix this, I realised there was a open strong tag on the div container before the footer.


- 'About Us' section is not centered vertically in the container so that it looks centered in the middle of the container box

    ![screenshot](documentation/bug05.jpg)
...
- Begin to lose part of the hero image when reducing the size of the screen (you can see in the screenshot i'm referring to the rocket).
...
| [Begin to lose part of the hero image when reducing the size of the screen](https://github.com/staceyJayneLewis/little-hatchlings/issues/2) | Closed |

**Open Issues**

There are no remaining open issues (https://github.com/staceyJayneLewis/little-hatchlings/issues).
